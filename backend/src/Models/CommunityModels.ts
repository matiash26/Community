import { ICommunity, IPost } from './type';
...
class Community implements ICommunity {
  ...
  async getUserLiked(userId: number, postId: number): Promise<unknown> {
    const mysql = await client();
  ...
  async getUserByEmailOrUser(emailOrUser: string): Promise<unknown> {
    const mysql = await client();
    const query =
